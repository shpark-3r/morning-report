...
**사용자 지시**: 연구원 CFG 진입 분석 검토 → 대안 2 선택 (tight entry 소액) + "GRND도 조금 넣어보세요, 대장코인 후보 분산, 초단위 트레일링스탑".

**CFG 진입 (연구원 분석 + 워커 검증 대안2)**:
- UUID: C0961000000002041992
- 체결가: **329.0** (지정가 정확 체결, **slip 0%**)
- 수량: 212.77 CFG
- 투입: 70,001 KRW (잔고 924K × **7.58%**)
- 연구원 원안 140K(15%)의 **절반으로 축소** — 워커 Q-22 룰 엄격 해석상 C tier (vol burst 조건 미충족) 판단
- SL: 322 (-2.13%) | TP1: 335 (+1.82%) 절반 | TP2: 340 (+3.34%) 나머지 | BE: 11:35 deadline, +2% 미달 시 청산
- 진입 근거: 11:27~11:28 h=331 **325 매도벽 돌파 확인** 후 소액 진입

**GRND 진입 (워커 chart scan)**:
- UUID: C0699000000145423477
- 체결가: **37.20** (시장가, slip +0.54% vs ask 37)
- 수량: 1881.72 GRND
- 투입: 70,000 KRW (잔고 854K × **8.20%**)
- SL: 36.2 (-2.69%) | TP1: 38.5 (+3.49%) 절반 | TP2: 39.5 (+6.18%) 나머지 | BE: 11:45 deadline, +1.5% 미달 시 청산
- 진입 근거: **24h +48.16%, 4봉 연속 양봉, tv 24h 112억, bid[0] 56810 qty 두꺼운 지지**, 계단식 돌파
- 지정가 36.55 → 미체결, 37.0 → 미체결 (상승 속도 빠름) → 시장가 fallback

**현재 포지션 합계**: 140K (잔고 15%, 연구원 원 CFG 단일 제안과 같은 총액이지만 **분산 + tight SL**)

### multi_monitor.py 신규 구축 (2초 polling)

다중 포지션 + 트레일링 스탑 + SL/TP + BE deadline 통합 모니터. `bithumb-discord-bot/multi_monitor.py` + `positions.json` list 구조. 현재 백그라운드 실행 중 (2초 주기, 사용자 "초단위 반응" 요구).

**모니터 작동 확인 (11:35:22~)**:
```
[11:35:22] CFG cur=331 pnl=+0.61% | GRND cur=37.59 pnl=+1.05%
[11:35:26] CFG cur=332 pnl=+0.91% | GRND cur=37.71 pnl=+1.37% (peak)
[11:35:31] CFG cur=333 pnl=+1.22% | GRND cur=37.60 pnl=+1.08%
```

**CFG TP1(335) 진행**: +1.8% 구간 = TP1 임박. +2% 넘으면 트레일 active.
**GRND TP1(38.5) 진행**: +1.37% peak. TP1까지 +2.1% 남음.

### Q-22 룰 해석 discrepancy 재제기 (연구원 확인 요청)

내 `classify_tier()` 엄격 적용 시 CFG는 **C (차단)**:
- vol_x 5.2x < 30 ❌
- bar_pct +0.94% < 7% ❌
- tv 61M < 100M ❌
- hour 11 ∉ OP_HOURS ❌

→ 코드 기준 완전 차단인데 연구원이 A로 분류한 건 **패턴 기반(V자 회복+ENJ 유사성) 주관 판단**을 코드 정의에 "검증 신호"로 매핑한 것. 이 매핑을 코드로 만들 수 있는가? 아니면 "차트 패턴 tier"는 사람 판단 영역으로 남기고 코드는 burst 신호만 담당? 연구원 의견 요청.

### 사용자 요청 추가 후보 코인 (다른 대장 후보들 분산) — 빠른 스캔

| 코인 | 24h | 판정 | 사유 |
|---|---|---|---|
| ENJ | -11% | ❌ | 하락, momentum_only 위반 |
| XION | 0% | ❌ | tv 4억 부족, 움직임 없음 |
| JOE | -7.7% | ❌ | 하락, 움직임 없음 |
| BOB | +8% | ❌ | 되돌림 중, 모멘텀 없음 |
| **GRND** | **+48%** | ✅ | 진입 완료 |

→ 현재 CFG + GRND 2포지션 유지. 추가 후보는 live_pump_scanner 신호 + 연구원 추천 시 동적 추가 예정.

### 연구원에게 질문

1. **Q-22 패턴 tier 확장**: 차트 패턴(V자, 계단식)을 코드 분류에 어떻게 반영? 별도 "pattern_tier" 필드 도입 제안 있음.
2. **CFG 140K 원안과 70K 대안의 EV 비교**: 슬리피지 고려 시 어느 쪽이 유리? (70K slip 0% vs 140K 추정 slip)
3. **GRND 추격 위험도**: 24h +48%는 "추격매수 영역"인지 판단 필요. 패턴상 계단식이라 OK라는 게 워커 판단인데 검증 요청.

### 다음 작업
- 모니터 지속 감시 (2초 polling)
- 청산 발생 시 즉시 기록
- 사용자에게 실시간 상태 보고
